...
public class PaseController implements Initializable {
	...
	@FXML
	private TextField idField, fechaField, horaField, peliIdField, cineIdField;
	...
	@FXML
	private void insert() {
		if (!fechaField.getText().isBlank() && !peliIdField.getText().isBlank() && !cineIdField.getText().isBlank()) {
			var fecha = fechaField.getText();
			var hora = horaField.getText();
			...
			repository.insert(new Pase(cine, pelicula, fecha, hora));
			showAll();
		}
	}
	...
	@FXML
	private void update() {
		if (!idField.getText().isBlank() && !fechaField.getText().isBlank() && !horaField.getText().isBlank()
				&& !peliIdField.getText().isBlank() && !cineIdField.getText().isBlank()) {
			var id = Integer.parseInt(idField.getText());
			var fecha = fechaField.getText();
			var hora = horaField.getText();
			var peliId = Integer.parseInt(peliIdField.getText());
			...
			repository.update(id, new Pase(cine, pelicula, fecha, hora));
			showAll();
			clear();
		}
	}
	...
	private void showPases(List<Pase> datos) {
		...
		TableColumn<Pase, String> columnaHora = new TableColumn<>("Hora");
		...
		columnaPelicula.setCellValueFactory(
		...
		tablaView.getColumns().addAll(columnaId, columnaFecha, columnaHora, columnaPelicula, columnaCineId);
