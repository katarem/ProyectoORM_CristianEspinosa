...
public class ProtagonistaController implements Initializable {
    ...
    private void select(){
        ...
        else if(!nombreField.getText().isBlank()) showByNombre();
        else showAll();
    }
    ...
    private void insert(){
        if(!nombreField.getText().isBlank() 
        	...
        	p.setPelicula(new Pelicula(peliculaId));
            repository.insert(p);
            showAll();
        }
    }
    ...
    private void showProtagonistas(List<Protagonista> datos){
        ...
        columnaPeliculaId.setCellValueFactory(cellData -> 
        	new SimpleIntegerProperty(cellData.getValue().getPelicula().getId()).asObject()
        );
        tablaView.getColumns().clear();
        tablaView.getColumns().addAll(columnaId,columnaNombre,columnaFoto,columnaPeliculaId);
